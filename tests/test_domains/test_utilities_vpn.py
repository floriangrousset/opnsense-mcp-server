...
class TestUtilitiesDomain:
    ...
    async def test_exec_api_call_get(self, mock_mcp_context):
        ...
        with patch(
            "src.opnsense_mcp.domains.utilities.get_opnsense_client", new_callable=AsyncMock
        ) as mock_get_client:
            mock_client = Mock()
            ...
            result = await exec_api_call(
                ctx=mock_mcp_context, method="GET", endpoint="/core/system/status"
            )
    ...
    async def test_exec_api_call_invalid_json(self, mock_mcp_context):
        ...
        with patch(
            ...
            result = await exec_api_call(
                ctx=mock_mcp_context, method="POST", endpoint="/test", data="invalid json{"
            )
...
class TestVPNDomain:
    ...
    async def test_get_vpn_connections(self, mock_mcp_context):
        ...
        with patch(
            "src.opnsense_mcp.domains.vpn.get_opnsense_client", new_callable=AsyncMock
        ) as mock_get_client:
            mock_client = Mock()
            # Function only makes ONE call for default vpn_type="OpenVPN"
            mock_client.request = AsyncMock(
                return_value={"rows": [{"name": "OpenVPN1", "status": "up"}]}
            )
            mock_get_client.return_value = mock_client
