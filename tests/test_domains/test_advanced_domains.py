...
class TestUsersDomain:
    ...
    async def test_list_users(self, mock_mcp_context):
        ...
        with patch(
            "src.opnsense_mcp.domains.users.get_opnsense_client", new_callable=AsyncMock
        ) as mock_get_client:
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={"rows": [{"uuid": "user1", "name": "admin", "disabled": "0"}]}
            )
    ...
    async def test_list_groups(self, mock_mcp_context):
        ...
        with patch(
            ...
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={"rows": [{"uuid": "group1", "name": "admins"}]}
            )
            mock_get_client.return_value = mock_client
...
class TestLoggingDomain:
    ...
    async def test_get_system_logs(self, mock_mcp_context):
        ...
        with patch(
            "src.opnsense_mcp.domains.logging.get_opnsense_client", new_callable=AsyncMock
        ) as mock_get_client:
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={
    ...
    async def test_search_logs(self, mock_mcp_context):
        ...
        with patch(
            ...
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={"rows": [{"message": "Authentication failed", "severity": "warning"}]}
            )
            mock_get_client.return_value = mock_client

            result = await search_logs(
                ctx=mock_mcp_context, search_query="authentication", case_sensitive=False
            )
...
class TestTrafficShapingDomain:
    ...
    async def test_traffic_shaper_list_pipes(self, mock_mcp_context):
        ...
        with patch(
            "src.opnsense_mcp.domains.traffic_shaping.get_opnsense_client", new_callable=AsyncMock
        ) as mock_get_client:
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={
    ...
    async def test_traffic_shaper_list_queues(self, mock_mcp_context):
        ...
        with patch(
            ...
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={"rows": [{"uuid": "queue1", "pipe": "1", "weight": "50"}]}
            )
            mock_get_client.return_value = mock_client
