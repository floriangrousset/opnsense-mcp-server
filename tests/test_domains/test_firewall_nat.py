...
class TestFirewallDomain:
    ...
    async def test_firewall_get_rules(self, mock_mcp_context, mock_http_success_response):
        ...
        with patch(
            ...
            mock_client.request = AsyncMock(
                return_value={"rows": [{"uuid": "rule1", "action": "pass"}], "rowCount": 1}
            )
    ...
    async def test_get_firewall_aliases(self, mock_mcp_context):
        ...
        with patch(
            ...
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={"rows": [{"name": "WebServers", "type": "host"}]}
            )
            mock_get_client.return_value = mock_client
...
class TestNATDomain:
    ...
    async def test_nat_list_outbound_rules(self, mock_mcp_context):
        ...
        with patch(
            "src.opnsense_mcp.domains.nat.get_opnsense_client", new_callable=AsyncMock
        ) as mock_get_client:
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={"rows": [{"uuid": "nat1", "source": "192.168.1.0/24"}]}
            )
            mock_get_client.return_value = mock_client
    ...
    async def test_nat_list_one_to_one_rules(self, mock_mcp_context):
        ...
        with patch(
            ...
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={"rows": [{"external": "203.0.113.10", "internal": "192.168.1.100"}]}
            )
            mock_get_client.return_value = mock_client
