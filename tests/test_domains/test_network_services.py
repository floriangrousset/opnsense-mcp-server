...
class TestNetworkDomain:
    ...
    async def test_list_vlans(self, mock_mcp_context):
        ...
        with patch(
            "src.opnsense_mcp.domains.network.get_opnsense_client", new_callable=AsyncMock
        ) as mock_get_client:
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={"rows": [{"uuid": "vlan1", "tag": "100", "if": "em0"}]}
            )
    ...
    async def test_create_vlan_interface(self, mock_mcp_context):
        ...
        with patch(
            ...
            result = await create_vlan_interface(
                ctx=mock_mcp_context, parent_interface="em0", vlan_tag=100, description="IoT VLAN"
            )
...
class TestDNSDHCPDomain:
    ...
    async def test_dhcp_get_leases(self, mock_mcp_context):
        ...
        with patch(
            "src.opnsense_mcp.domains.dns_dhcp.get_opnsense_client", new_callable=AsyncMock
        ) as mock_get_client:
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={
    ...
    async def test_dns_resolver_get_settings(self, mock_mcp_context):
        ...
        with patch(
            ...
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={"unbound": {"enabled": "1", "port": "53"}}
            )
            mock_get_client.return_value = mock_client
...
class TestCertificatesDomain:
    ...
    async def test_list_certificates(self, mock_mcp_context):
        ...
        with patch(
            "src.opnsense_mcp.domains.certificates.get_opnsense_client", new_callable=AsyncMock
        ) as mock_get_client:
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={"rows": [{"uuid": "cert1", "descr": "Web Server Cert"}]}
            )
            mock_get_client.return_value = mock_client
    ...
    async def test_list_certificate_authorities(self, mock_mcp_context):
        ...
        with patch(
            ...
            mock_client = Mock()
            mock_client.request = AsyncMock(
                return_value={"rows": [{"uuid": "ca1", "descr": "Root CA"}]}
            )
            mock_get_client.return_value = mock_client
