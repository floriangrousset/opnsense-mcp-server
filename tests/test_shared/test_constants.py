...
class TestAPIConstants:
    ...
    def test_all_constants_non_empty(self):
        ...
        for constant in api_constants:
            assert constant != "", "Constant should not be empty"
    ...
    def test_constants_module_has_expected_count(self):
        ...
        api_constants = [
            attr for attr in dir(constants) if attr.startswith("API_") and not attr.startswith("__")
        ]

        # Should have at least 100 constants (we have 166 tools, so likely many endpoints)
        assert (
            len(api_constants) >= 100
        ), f"Expected at least 100 API constants, got {len(api_constants)}"
    ...
    def test_traffic_shaper_pipe_search_endpoint(self):
        ...
        assert (
            constants.API_TRAFFICSHAPER_SETTINGS_SEARCH_PIPES
